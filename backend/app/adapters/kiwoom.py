...
from ..services.themes import CODE_NAME as _CODE_NAME, THEME_CODES as _THEME_CODES
...
# 테마 정의는 services/themes.py가 단일 소스 (19테마 95종목, 자금 흐름 수집과 공유).
# 기존 형태 (theme, [codes]) 로 어댑터에 맞춰 노출한다.
_THEME_SECTORS: list[tuple[str, list[str]]] = [(t, list(cs)) for t, cs in _THEME_CODES.items()]
_MAJOR_NAMES: dict[str, str] = dict(_CODE_NAME)
