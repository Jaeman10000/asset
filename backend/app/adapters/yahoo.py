...
# 섹터 등락률은 전일 종가 기준이라 자주 안 변한다 — 5분 캐시 + 실패 시 stale 반환.
# 7초 폴링 × 11심볼이 그대로 Yahoo로 나가면 레이트리밋/차단 위험(상시 위젯).
_SECTOR_TTL = 300.0
_sector_cache: tuple[float, list[SectorFlow]] | None = None  # (ts, flows)
...
class YahooAdapter(BaseAdapter):
    ...
    async def fetch(self) -> AdapterResult:
        ...
        if _sector_cache and now - _sector_cache[0] < _SECTOR_TTL:
            return AdapterResult(sector_flows=_sector_cache[1], background=True)
        ...
        try:
            ...
            if _sector_cache:  # stale-on-error: 만료됐어도 옛 데이터가 빈 것보다 낫다
                return AdapterResult(sector_flows=_sector_cache[1], background=True)
        ...
        if not flows:
            if _sector_cache:  # stale-on-error
                return AdapterResult(sector_flows=_sector_cache[1], background=True)
            return AdapterResult(
        ...
        if failed:
            ...
            _sector_cache = (now, flows)  # 전체 성공일 때만 캐시 (부분 성공은 다음에 재시도)

        return AdapterResult(sector_flows=flows, error=error, background=True)
