...
TARGET_SEC = 168.0     # 쇼츠 한계 180초 - 여유. 넘기면 쇼츠 피드에서 빠진다.
# 2026-09-14 JJ: "2분 넘어도 3분이 되도 상관없다. 어차피 볼 사람은 본다. 짧아서 끝까지 본다고
# 좋은 영상이 아니다. 구독자가 늘려면 그만큼 가치 있는 정보를 줘야 한다." 그래서 길이로 자르지 않고,
# 쇼츠에서 빠지지 않을 선까지만 남긴다. 대신 끝까지 보게 만드는 건 트리거로 만든다.
                       # 3분은 "쇼츠에서 빠지지 않는 상한"이지 목표가 아니었다. 2분을 넘으면 PRIO 큰 문장부터 빠진다.
MIN_SEC = {"w0": 5.0, "w1": 8.0, "w2": 8.0, "w3": 8.0, "w4": 6.0, "w5": 5.0, "w6": 6.0}
# 덜 중요한 문장부터 뺀다(큰 번호 먼저). 여기 없는 문장은 필수.
# w4(뉴스 연결)는 2분을 넘을 때만 뺀다(JJ 2026-09-14) — 먼저 버리지 않고 다른 곁가지를 먼저 정리한다.
# 기록의 '그래서 뭔데'(w6.mean)는 JJ가 요청한 것이라 맨 마지막에 뺀다.
PRIO = {"w1.near": 8, "w1.held": 8,
        "w3.stay_small": 7, "w2.total2": 7,
        "w6.cond": 6, "w3.bq": 6, "w3.ba": 6, "w3.concl": 6,
        "w1.touched": 5, "w3.link": 5, "w2.opp": 5,
        "w3.rev_in": 4,
        "w4.ev": 3, "w4.close_link": 3, "w4.close": 2, "w4.q": 2,
        "w6.fails": 1, "w6.mean": 1}
...
def scene_w6(F: dict, pk: Picker) -> list:
    ...
    # ② 시청자에게 묻는다 ③ 좋아요 — 댓글 0개, 좋아요율 0.05~0.6%의 원인은 한 번도 말을 걸지 않은 것(2026-09-14)
    P.append((pk("w6.ask", ["이번 주 이 돈의 움직임, 여러분은 어떻게 보셨습니까? 댓글로 남겨 주세요.",
                            "여러분이 본 이번 주는 어땠습니까? 댓글로 남겨 주세요.",
                            "이번 주 숫자에서 가장 이상했던 건 무엇이었습니까? 댓글로 알려 주세요."]), "w6.ask"))
    P.append((pk("w6.cta", ["도움이 되셨다면 좋아요, 다음 주가 궁금하면 구독 눌러 주세요.",
                            "이 정리가 도움이 됐다면 좋아요와 구독 부탁드립니다.",
                            "매주 이 자리에서 세어 드립니다. 좋아요와 구독 눌러 주세요."]), "w6.cta"))
