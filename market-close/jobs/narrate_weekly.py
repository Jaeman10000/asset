...
# ── Threads(새 형식) 한도와 검사 ──
TH_MAX_CHARS = 500          # 태그까지 포함한 한 편(상한)
...
TH_END_RE = re.compile(r"(?:니다[.]?|까요[?]?|[억조%][.]?|,)$")                       # 완결형이거나(숫자로 끝나는 짧은 절 포함) 다음 줄로 이어지는 줄
TH_SHORT_FIG_RE = re.compile(r"\d+\.\d+\s?조")                                        # '9.9조' 같은 압축 표기 금지
TH_MIN_CHARS = 300          # 이보다 짧으면 검사 실패(목표는 TH_AIM_CHARS 350~420)
...
# 첫 줄에 있어야 하는 '이미 봤다'는 인정
TH_SEEN_RE = re.compile(r"(보셨|아실|보는 숫자|보이는 숫자|이미|누구나|기사 제목)")
# 사실 없이 다음을 예고만 하는 빈손 문장 — 첫 문단에 있으면 실패
TH_TEASE = ["확인할 숫자", "따로 있습니다", "따로 있었습니다", "말씀드리", "알려 드리", "밑에 적", "아래에 적"]
...
def _flip_bank(F: dict, G: str, mate: str = "") -> tuple[list, dict]:
    """방향을 바꾼 주체 — 완결형 문장 뱅크. mate가 있으면 '그쪽만 멈추지 않았다'로 대비를 준다."""
    f = F["flips"][G]
    e_buy = f["early"] > 0
    ...
    solo = ["{G_eun} {start}부터 {late} 쪽으로 돌아섰습니다.",
            "{G_eun} 주 초반엔 {early} {start}부터 {late} 쪽이었습니다.",
            "{G_iga} {late} 쪽으로 돌아선 건 {start}부터입니다."]
    if not mate:
        return solo, kw
    return [("{G_eun} {start}부터 {late} 쪽으로 돌아섰는데,", "{M_man} 하루도 멈추지 않았습니다."),
            ("{G_eun} {start}부터 {late} 쪽으로 돌아섰습니다.", "그 사이 {M_man} 자리를 지켰습니다."),
            ("{start}부터는 {G_iga} {late} 쪽으로 바뀌었는데,", "{M_iga} 멈춘 날은 하루도 없었습니다.")] + [(s,) for s in solo], kw
...
def _seen_bank(C: float) -> list:
    ...
    if C >= 0.1:
        return [("한 주가 어떻게 끝났는지는 이미 보셨을 겁니다.", "코스피는 {C} 올랐습니다."),
                "코스피가 {C} 오른 한 주였다는 건 아실 겁니다.",
                ("한 주를 마치고 앱을 열면 코스피가 {C} 올라 있습니다.", "여기까지는 누구나 보는 숫자입니다."),
                "이번 주 코스피 {C} 상승까지는 뉴스에서 보셨을 겁니다.",
                ("주말 기사 제목은 다 비슷합니다.", "코스피 {C} 상승, 오른 한 주였다는 이야기입니다."),
                ("코스피가 {C} 올라 한 주를 끝냈습니다.", "여기까지는 다 보이는 숫자입니다.")]
    if C <= -0.1:
        return [("한 주가 어떻게 끝났는지는 이미 보셨을 겁니다.", "코스피는 {C} 내렸습니다."),
                "코스피가 {C} 내린 한 주였다는 건 아실 겁니다.",
                ("한 주를 마치고 앱을 열면 코스피가 {C} 내려 있습니다.", "여기까지는 누구나 보는 숫자입니다."),
                "이번 주 코스피 {C} 하락까지는 뉴스에서 보셨을 겁니다.",
                ("주말 기사 제목은 다 비슷합니다.", "코스피 {C} 하락, 내린 한 주였다는 이야기입니다."),
                ("코스피가 {C} 내려 한 주를 끝냈습니다.", "여기까지는 다 보이는 숫자입니다.")]
    return [("한 주가 어떻게 끝났는지는 이미 보셨을 겁니다.", "코스피는 제자리였습니다."),
            "코스피가 제자리로 끝난 한 주였다는 건 아실 겁니다.",
            ("한 주를 마치고 앱을 열어도 코스피는 그대로입니다.", "여기까지는 누구나 보는 숫자입니다."),
            "이번 주 코스피가 거의 안 움직인 건 보셨을 겁니다.",
            ("주말 기사 제목은 다 비슷합니다.", "코스피는 시작한 자리 근처, 조용한 한 주였다는 이야기입니다."),
            ("코스피가 제자리에서 한 주를 끝냈습니다.", "여기까지는 다 보이는 숫자입니다.")]
...
def build_threads(F: dict, pk: Picker, htype: str) -> tuple[str, str]:
    ...
    그 뒤는 (2) 그럼 반대편은 누구였나 → (3) 그래서 무슨 뜻. 중간에 질문 한 줄.
    완결형 '-습니다/-입니다'체, 숫자는 풀어 쓰고(9조 9천억), 우리만 아는 카운터는 말로 푼다.
    ...
    if side == "S":
        bank = ["그런데 그 {WKN}에 {S_eun} {X_eul} 팔았습니다.",
                "그 {WKN}에 가장 많이 판 쪽은 {S}, {X}입니다.",
                "그런데 {WDUR} {S_iga} 판 돈이 {X}입니다.",
                "그 한 주 안에서 {S_eun} {X_eul} 내다 팔았습니다.",
                "그런데 같은 기간 {S_eun} {X_eul} 팔고 나갔습니다.",
                "{WKN}였는데, {S_eun} 그 사이 {X_eul} 팔았습니다."]
    elif side == "B":
        bank = ["그런데 그 {WKN}에 {B_eun} {Y_eul} 사들였습니다.",
                "그 {WKN}에 가장 많이 산 쪽은 {B}, {Y}입니다.",
                "그런데 {WDUR} {B_iga} 산 돈이 {Y}입니다.",
                "그 한 주 안에서 {B_eun} {Y_eul} 받아 갔습니다.",
                "그런데 같은 기간 {B_eun} {Y_eul} 사 모았습니다.",
                "{WKN}였는데, {B_eun} 그 사이 {Y_eul} 샀습니다."]
    elif side == "G":
        kw = {**kw, **_flip_bank(F, F["G"])[1]}
        bank = [("그런데 그 {WKN} 안에서,", "{G_eun} {start}부터 {late} 쪽으로 돌아섰습니다."),
                ("그 {WKN}의 한가운데서 방향이 한 번 바뀌었습니다.", "{G_iga} {start}부터입니다."),
                ("그런데 {WDUR} 방향을 바꾼 쪽이 있습니다.", "{G_eun} {start}부터 {late} 쪽이었습니다."),
                ("{WKN}였는데, 그 안은 한 방향이 아니었습니다.", "{G_eun} {start}부터 {late} 쪽으로 돌아섰습니다."),
                ("그런데 그 한 주가 내내 한 방향이지는 않았습니다.", "{G_iga} {start}부터 {late} 쪽으로 바뀌었습니다."),
                ("그 {WKN} 안에서 {G_eun} 중간에 손을 바꿨습니다.", "{start}부터입니다.")]
    else:
        bank = ["그런데 그 {WKN} 안에서는 사는 쪽과 파는 쪽이 갈렸습니다.",
                "그 {WKN} 안을 열어 보면 한쪽으로 기운 곳이 없습니다.",
                "그런데 {WDUR} 크게 사거나 판 쪽은 보이지 않습니다.",
                "그 한 주 안에서는 어느 쪽도 크게 움직이지 않았습니다.",
                "{WKN}였는데, 산 쪽과 판 쪽이 서로 엇갈렸습니다.",
                "그런데 같은 기간 돈은 한쪽으로 모이지 않았습니다."]
    ...
    # (2) 그래서 반대편은 누구였나
    if side == "S":
        qbank = ["그 주식은 누가 받았을까요?", "그럼 그 주식은 누가 넘겨받았을까요?", "판 쪽이 있으면 받은 쪽도 있습니다. 어디였을까요?"]
    elif side == "B":
        qbank = ["그 주식은 누가 넘긴 걸까요?", "그럼 판 쪽은 어디였을까요?", "산 쪽이 있으면 판 쪽도 있습니다. 어디였을까요?"]
    else:
        qbank = ["그 사이 돈은 어디로 움직였을까요?", "그럼 돈은 어디로 갔을까요?", "이 한 주에 무슨 일이 있었을까요?"]
    ask = _th_lines(pk("th.q", qbank, threads=True, ok=_th_fit))
    if side == "B":                                  # 첫 문단이 산 쪽을 말했으니 여기는 판 쪽
        mate = S
        if steady_s:
            bank = ["{Nd} 내내 판 곳이 하나 있었습니다. {S}, {X}.",
                    "하루도 쉬지 않고 내놓은 곳이 있습니다. {S}, 한 주 {X}.",
                    "{N} 내내 판 쪽은 {S_ieot}습니다. 한 주 {X}."]
        elif tot[S] < 0:
            bank = ["그 주식을 내놓은 곳은 {S}, 한 주 {X}입니다.",
                    "넘긴 쪽은 {S_ieot}습니다. 한 주 합계 {X}.",
                    "한 주 동안 가장 많이 판 곳은 {S}, 금액은 {X}입니다."]
        else:
            bank = ["이번 주에는 네 곳 모두 산 쪽에 가까웠습니다.", "한 주 동안 크게 내놓은 쪽은 보이지 않았습니다.",
                    "이번 주에는 어느 쪽도 크게 팔지 않았습니다."]
    else:
        mate = B
        if steady_b:
            bank = ["{Nd} 내내 산 곳이 하나 있었습니다. {B}, {Y}.",
                    "하루도 쉬지 않고 산 곳이 있습니다. {B}, 한 주 {Y}.",
                    "{N} 내내 사들인 쪽은 {B_ieot}습니다. 한 주 {Y}."]
        elif tot[B] > 0:
            bank = ["그 주식을 가장 많이 받은 곳은 {B}, 한 주 {Y}입니다.",
                    "받아 간 쪽은 {B_ieot}습니다. 한 주 합계 {Y}.",
                    "한 주 동안 가장 많이 산 곳은 {B}, 금액은 {Y}입니다."]
        else:                                        # 네 주체가 모두 판 쪽 — 금액을 붙이면 거꾸로 읽힌다
            bank = ["이번 주에는 네 곳 모두 판 쪽에 가까웠습니다.", "한 주 동안 크게 받아 간 쪽은 보이지 않았습니다.",
                    "이번 주에는 어느 쪽도 크게 받아 가지 않았습니다."]
    ...
    if F["bb"]:
        ...
        stock = _th_lines(pk("th.stock", [
            ("가장 많이 산 주식은 {most} {names_ieot}고,", "{who_ga} 자기 회사 주식을 사 모으는 중입니다."),
            ("{most} {names}에 들어간 돈입니다.", "{who_all} {M}월부터 자기 회사 주식을 사고 있습니다."),
            ("사들인 주식은 {most} {names_ieot}습니다.", "{who_ga} 자기 회사 주식을 사들이고 있습니다.")], threads=True, ok=_th_fit, **skw))
    ...
    if stay:
        ...
        pair = [("외국인과 기관 돈이 한 주 내내 머문 곳은 {TS_ieot}습니다.", "{TO_eun} 거꾸로 한 주 내내 빠지는 쪽이었습니다."),
                ("분야로 보면 {TS_en} 돈이 계속 들어왔습니다.", "{TO}에서는 거꾸로 돈이 나갔습니다."),
                ("끝까지 돈이 남아 있던 분야는 {TS_ieot}습니다.", "가장 많이 빠져나간 분야는 {TO}입니다.")]
        solo = ["외국인과 기관 돈이 한 주 내내 머문 곳은 {TS_ieot}습니다.",
                "분야로 보면 {TS_en} 돈이 계속 들어왔습니다.",
                "끝까지 돈이 남아 있던 분야는 {TS_ieot}습니다."]
        sector = _th_lines(pk("th.sector", (pair + [(s,) for s in solo]) if out_th else [(s,) for s in solo],
                              threads=True, ok=_th_fit, **tkw))
    elif out_th:
        sector = _th_lines(pk("th.sector_out", ["분야로 보면 돈이 가장 많이 빠진 곳은 {TO}입니다.",
                                                "외국인과 기관 돈이 가장 크게 나간 분야는 {TO}입니다.",
                                                "한 주 동안 돈이 가장 많이 빠져나간 곳은 {TO}입니다."],
                              threads=True, ok=_th_fit, TO=out_th))
    ...
    if F["checks"]:
        ...
        if k == 0:
            bank = ["하루 들어온 돈이 다음 날까지 남은 자리는 이번 주에 없었습니다.", "한 번 잡힌 방향은 이번 주 전부 하루로 끝났습니다.",
                    "이번 주엔 다음 날까지 이어진 방향이 하나도 없었습니다."]
        elif k < n:
            bank = ["하루 만에 끊긴 자리도 있고, 다음 날까지 간 자리도 있었습니다.", "한 번 잡힌 방향이 다음 날까지 간 자리는 일부뿐이었습니다.",
                    "이번 주엔 다음 날까지 이어진 방향이 일부만 있었습니다."]
        else:
            bank = ["하루 들어온 돈이 다음 날까지 남은 자리가 이번 주엔 전부였습니다.", "한 번 잡힌 방향은 이번 주 모두 다음 날까지 갔습니다.",
                    "이번 주엔 잡힌 방향이 전부 다음 날에도 그대로였습니다."]
        rec = _th_lines(pk("th.record", bank, threads=True, ok=_th_fit))
    # 마지막 문단은 '그래서 무슨 뜻'으로 닫는다. 다음에 볼 것·채널 고지는 답글 몫이라 여기서 예고하지 않는다(JJ 2026-09-13).
    if F["bb"] and steady_b:
        bank = [("회사가 자기 회사 주식을 사들이는 동안에는,", "파는 주식을 받아 줄 곳이 하나 있다는 뜻입니다."),
                ("이 돈은 값이 싸 보여서 들어온 돈이 아닙니다.", "회사가 미리 정해 둔 기간 동안 사는 돈입니다."),
                ("파는 쪽이 내내 나와도 받아 준 곳이 있었습니다.", "한쪽으로만 기울지 않은 한 주였다는 뜻입니다.")]
    else:
        bank = [("{WKN}라는 한 줄 뒤에,", "서로 다른 쪽으로 움직인 돈이 있었다는 뜻입니다."),
                ("코스피 숫자만 보면 여기까지입니다.", "그 안에서 돈의 주인이 바뀐 한 주였다는 뜻입니다."),
                ("숫자 한 줄로는 오른 주, 내린 주만 남습니다.", "누가 사고 누가 팔았는지는 그 안에 있습니다.")]
    ...
    reply = ("영상 전체는 여기서 → {YT}\n"
             "평일에는 매일 오후 4시 30분, 그날 한국 시장에서 누가 샀는지 올립니다."
             + (f"\n다음 주에는 {q_full} 보겠습니다." if q_full else ""))
    return body + "\n" + TAG, reply
...
def validate(out: dict, F: dict, hit=None) -> list[str]:
    ...
    for x in body:
        ...
        if TH_CLIP_RE.search(x.rstrip()):
            ...
            issues.append(f"threads 완결형 문장 아님: {x}")
        if re.match(r"^\s*\d+\s*[.)]\s", x):
            issues.append(f"threads 번호 목록: {x}")
    if not any(x.endswith(("니다.", "니다")) for x in body):
        issues.append("threads 합니다체 문장 없음")
    btxt = "\n".join(body)
    ...
    if blocks and re.search(r"(다음 주|월요일|보겠습니다|보려고 합니다|확인해 보)", blocks[-1]):
