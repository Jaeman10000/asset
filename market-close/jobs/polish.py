...
- "threads" 항목은 영상 대본이 아니라 운영자(개인 투자자)가 자기 계정에 올리는 글이다. 초안(threads)은 사실 메모일 뿐이니 그 사실을 바탕으로 **사람이 친구에게 오늘 장을 말해 주듯** 다시 쓴다:
  · 해요체. 1인칭 허용("저는 오늘 이렇게 봤어요"). 브리핑체("~했습니다. ~였습니다." 나열) 금지.
  · 첫 줄은 숫자 없이, 오늘 장을 본 느낌이나 관찰 한 줄(예: "개인이 또 팔았는데 지수는 올랐어요. 오늘도 그 그림이었습니다.").
  · 숫자는 글 전체에 최대 3개(억·조·%)만. 초안에 있는 숫자만 쓴다. 초안의 숫자를 다른 표현으로 바꾸지 말 것('33거래일' → '두 달 만에' 같은 환산·어림 금지). 뺄 수는 있어도 바꿀 수는 없다.
  · 가운데는 '오늘의 해석' 한두 줄: 돈이 어디서 어디로 갔는지, 어제 보자고 한 게 이어졌는지 끊겼는지, 그게 무슨 뜻인지(완곡하게: ~로 보여요, ~일 수 있어요). 이슈(예: 애플 발표)가 초안에 있으면 한 줄로 엮는다.
  · 내일 볼 것 한 줄("내일은 ○○가 이틀째 이어지는지만 봅니다").
  · 마지막 줄은 반드시 읽는 사람에게 던지는 질문 한 줄로 끝낸다(물음표). 답글이 달리게.
  · 매수·매도 권유, 종목 추천, 수익·확률·부자 표현, 이모지, 말줄임표, 해시태그 금지(해시태그는 코드가 붙인다). 문단 사이는 빈 줄. 전체 450자 이내.
...
def polish(d: str, ed: str = "kr") -> dict:
    ...
    if th:
        ...
        # 새 형식(2026-09-13): 1줄은 '독자가 이미 본 지수'라 숫자가 반드시 들어가고, 마지막 줄은 질문이 아니라 '뜻'이다.
        # 옛 조건(첫 줄 숫자 금지 / 마지막 줄 물음표 / 숫자 4개)이 그대로 남아 매일 세 개가 동시에 걸렸다 — 다듬기 경로가 죽어 있었다.
        if not (330 <= len(th) <= 430):
            why.append(f"{len(th)}자 (330~430 밖)")
        if lines_th and not re.search(r"\d", lines_th[0]):
            why.append("첫 줄에 지수 숫자가 없음")
        if lines_th and lines_th[-1].rstrip().endswith("?"):
            why.append("마지막 줄이 질문 — '그래서 무슨 뜻'으로 닫는다")
