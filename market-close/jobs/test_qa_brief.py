...
S6_BODY = ("외국인 순매도가 6일째 이어지는지. 오늘까지 5일째라, 하루 더 이어져야 흐름으로 굳어진 것으로 봅니다. "
           "이차전지 유입이 2일째인지. 목요일 새벽 3시 미국 금리 결정입니다. ")   # 기준 문장 필수(JJ 2026-09-22, S6_CRITERION)

GOOD = [
    {"id": "s0", "tts": "외국인 1조 5,700억 순매도. 코스피는 0.85%만 내렸습니다."},
    {"id": "s1", "tts": "그 돈이 어디로 갔느냐. 오늘은 이것 하나만 봅니다."},
    {"id": "s2", "tts": "새 돈이라고 생각하기 쉽습니다. 외국인 1조 5,700억 순매도, 5일째입니다. 기관 9천억 순매도, 개인 8,300억 순매수입니다. "
                        "그런데 여기서 눈에 띄는 게 하나 있습니다. 기타법인 1조 6,400억, 1위입니다. 코스닥은 어땠을까요."},
    {"id": "s3a", "tts": "코스닥은 반대였습니다. 외국인 240억, 기관 1,100억 순매수였습니다. 개인은 1,400억 팔았습니다. "
                         "코스피 0.85% 하락, 코스닥 0.70% 상승입니다. 그런데 코스닥 외국인·기관은 반대로 샀습니다. 어느 업종에서 나갔을까요."},
    {"id": "s3b", "tts": "가장 많이 빠진 곳은 반도체입니다. 외국인 1조 3,400억, 기관 7,100억이 나갔습니다. 합쳐 2조 400억, 5일째입니다. "
                         "그런데 삼성전자는 0.2%, SK하이닉스는 0.4%만 내렸습니다. 자사주 1조 6,300억이 값을 붙든 겁니다. 어디로 옮겨 갔을까요."},
    {"id": "s3c", "tts": "이번엔 들어온 곳입니다. 이차전지에 외국인 480억, 기관 460억이 들어왔습니다. 로봇에는 외국인 260억, 기관 120억입니다. "
    ...
    {"id": "s4", "tts": "9월 15일 키움 종목별 투자자 표를 직접 열어봤습니다. 대장주 LG에너지솔루션은 3.98% 올랐습니다. "
                        ...
                        "외국인 13억, 기관 26억 순매수입니다. 개인 37억 순매도, 거래대금 548억입니다. 둘을 합친 39억은 7.1%뿐입니다."},
    {"id": "s5", "tts": "뉴스는 둘, 이차전지는 미국의 포드 서한, 로봇은 정부 AI 행사입니다. 뉴스가 올린 값이면 외국인·기관 돈이 작고, 돈이 올린 값이면 외국인과 기관이 같이 삽니다. "
                        "이차전지는 돈이 올렸고, 로봇은 뉴스에 올랐습니다. 뒤집히는 조건은 내일 로봇에 외국인·기관 돈 1,000억입니다. 처음 1조 5,700억 중 1조 3,400억이 반도체였습니다."},
...
SHORT_DROP = {"s3a": ("개인은 1,400억 팔았습니다.",),
              "s3b": ("합쳐 2조 400억, 5일째입니다.", "자사주 1조 6,300억이 값을 붙든 겁니다."),
              ...
              "s6": ("이차전지 유입이 2일째인지.", "외국인 순매도가 6일째 이어지는지.")}
...
class Passing(unittest.TestCase):
    ...
    def test_fixed_chain_facts_in_order(self):
        ...
        order = [("s2", "1조 5,700억"), ("s2", "9천억"), ("s2", "8,300억"), ("s2", "기타법인 1조 6,400억"),   # 99% 자사주 문장은 뺐다 — 기타법인은 이름·금액 한 문장(JJ 2026-09-21)
                 ...
                 ("s5", "포드"), ("s5", "로봇"), ("s5", "뉴스에 올랐"), ("s5", "뒤집히는 조건"),
                 ("s6", "6일째"), ("s6", "2일째"), ("s6", "새벽 3시")]
        last, pos = "", 0
...
class S2(unittest.TestCase):
    ...
    def test_others_required_unless_small(self):
        scenes = edit("s2", "새 돈이라고 생각하기 쉽습니다. 외국인 1조 5,700억 순매도, 5일째입니다. 기관 9천억 순매도, 개인 8,300억 순매수입니다. "
...
class S4(unittest.TestCase):
    ...
    def test_parties_and_no_indiv_fallback(self):
        no_indiv = ("9월 15일 키움 종목별 투자자 표를 직접 열어봤습니다. 대장주 LG에너지솔루션은 3.98% 올랐습니다. "
                    ...
                    "외국인 13억, 기관 26억 순매수입니다. 거래대금은 548억입니다. 둘을 합친 39억은 7.1%뿐입니다.")
...
class S5(unittest.TestCase):
    ...
    def test_verdict_and_condition_still_checked(self):
        bad = run(edit("s5", "뉴스는 둘, 이차전지는 미국의 포드 서한, 로봇은 정부 AI 행사입니다. "
                             "이차전지와 로봇 모두 올랐습니다. 주체별 합계라 종목 사이 이동은 안 보입니다."))
...
class Overlap(unittest.TestCase):
    def test_exact_overlap_from_previous_edition(self):
        recs = [{"kind": "kr", "date": "20260914", "folder": "20260914", "draft": False,
                 "scenes": [{"id": "s5", "tts": "이차전지는 돈이 올렸고, 로봇은 뉴스에 올랐습니다. 누가샀나였습니다."}]}]
        bad = run(recs=recs)
        hit = [b for b in bad if "글자 그대로 겹침" in b]
        self.assertEqual(len(hit), 1, bad)
        self.assertTrue(hit[0].startswith("[s5]") and hit[0].endswith(" :: 이차전지는 돈이 올렸고, 로봇은 뉴스에 올랐습니다."), hit)
