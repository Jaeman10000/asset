...
### 2-1. 요일별 '오른 종목의 이슈' — 주간편 v2 필수 (JJ 2026-09-17)
주간 대본은 요일마다 "그날 크게 오른 종목이 **이벤트(뉴스)로 오른 건지, 진짜 돈이 들어온 건지**"를 말한다. 돈 판정(외국인·기관 숫자)은 코드가 하고, 여기서는 **이슈만** 찾는다.
- 대상: `computed_weekly.json` 의 `days_detail[]` — 날마다 `risers`(그날 +5% 이상 오른 종목, 최대 3) 와 `ins[0].top_stocks`(돈이 들어온 업종의 상위 종목).
- 그 종목이 **그날** 오른 이유를 2곳 이상이 같이 쓴 기사로 찾는다. 1곳뿐이면 `confidence:"medium"`, 못 찾으면 넣지 않는다(억지로 만들지 않는다).
- 이벤트에 다음 필드를 **꼭** 채운다:
  - `stock`: 종목 이름(데이터와 같은 표기, 예 "LIG디펜스앤에어로스페이스")
  - `theme`: 우리 업종 이름(예 "방산")
  - `reason`: 대본에 그대로 들어갈 **짧은 명사구(20자 이내)** — 예 "UAE 통합 대공망 구축 기대", "낙폭과대 평가", "스페이스X 스타십 궤도 비행". 대본은 "{종목}은 {reason} 소식에 N% 올랐고…"로 읽는다.
- `reason` 에 넣지 않는 것: 기사 원제목, 목표주가·투자의견·증권사 추천, 금지어(때문에·덕분·영향으로…), 제재·소송·사고처럼 오른 이유가 아닌 기사, '특징주'·'강세'처럼 올랐다는 말만 되풀이하는 것("조선 테마 상승세").
