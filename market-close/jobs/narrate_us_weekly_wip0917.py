...
형식(JJ 2026-09-17): "미국 주식 주간 브리핑도 수급을 말할 필요는 없고 이벤트 사건 뉴스 등과 엮어서 어떤 주였는지 말해주는걸로 하자."
  → 사건·뉴스가 이야기를 끌고, 숫자는 그 사건 옆에 '같은 날'로 나란히 놓는다. 수급 낱말(외국인·기관·개인·순매수·
    돈이 머물/빠진)과 '월요일 국장에서 볼 것'은 쓰지 않는다. 정본 문서: docs/US_WEEKLY_GUIDE.md
  uw0 훅 = 그 주 가장 큰 사건 + 같은 날 시장 반응 + 한 주 지수와의 부딪힘 → 질문
  uw1 요일별 사건과 지수 한 주 흐름     uw2 물가·지표(실제 vs 추정치)·인상/인하 확률·금리
  uw3 달러·엔·원·금·유가가 사건 날 어떻게 움직였나     uw4 정치·지정학·기업 이슈(최대 3건, 날짜·출처)
  uw5 업종·대형주 — 어떤 사건 옆에서 움직였나(가격 기준)     uw6 '그래서 어떤 주였나' 한 줄 + 다음 주 일정 + 끝 멘트

입력
  w    : data/weekly_us/<build>/computed_us_weekly.json (공유 스키마. _meta가 있으면 결측 세션·휴장 이름에 쓴다)
  news : data/weekly_us/<build>/news_verified.json(없으면 news.json) — {"events": [...], "releases": [...]} 또는 이벤트 리스트.
         verified=false · use_in_script=false 인 기사는 여기서도 뺀다. 필드 이름은 넓게 받는다.
         event   : d|date(현지 날짜) · session(반영된 정규장) · title · spoken_ko(읽을 한 문장, 있으면 우선) · short_ko(짧은 이름)
                   · what|fact_ko(사실 문장) · sources[{outlet}] · market_link · confidence(high/medium/low)
                   · kind(geopolitics/politics/trade/policy/company/earnings/data/rates/fx/oil/market) · importance · assets
...
출력 {"scenes":[uw0..uw6 {id,min,tts,sub,screen}], "title", "threads", "threads_reply", "hook_parts":[훅 숫자, 훅 질문],
      + "variants"(슬롯별 고른 변형 번호), "complete", "missing", "hook", "checks", "total_chars", "est_sec"}
      scene.screen = 그 장면 화면에 쓸 데이터(사건 이름·지표 카드·일정). run_weekly 가 scenes 를 props 로 그대로 넘기므로
      WeeklyUSV4.tsx 가 p.scenes[i].screen 으로 읽는다.
      기사 없는 주는 uw0이 숫자 훅으로 돌아가고 uw4를 빼고 uw3의 질문이 uw5로 바로 잇는다.
      금요일 세션이 아직 없으면 uw1이 그 사실을 말하고 제목 날짜도 목요일까지.
길이: 장면별 한도(SCENE_CAP) + 전체 TOTAL_CAP(약 170초) — 넘으면 우선순위 낮은 문장부터 뺀다(0=필수).
지난주 기억: build_us_weekly 가 data/weekly_us/<빌드>/narration_us_weekly.json 을 남기고(prev=_AUTO 일 때),
  다음 주엔 그 파일(고른 변형) + run_weekly 가 쓴 script.json(JJ 수정까지 반영된 최종 대사)을 함께 읽어 같은 문장을 피한다.
...
Run:  python narrate_us_weekly.py [build_date=20260913] [--save]   (파일에 쓰지 않고 보기만 하려면 --save 없이)
"""
...
SIGN_OFF = "누가샀나 미국 주간 결산이었습니다. 평일엔 매일 저녁 5시에 국장 마감이 올라옵니다."
TAG = "#미장"
YT = "{YT}"
...
KR_RE = re.compile(r"코스피|코스닥|국고채|기타법인|네 마녀|원·달러|원/달러|서울 외환|자사주")
# 미국 주간엔 수급 낱말을 쓰지 않는다(JJ 2026-09-17). 기사 문장에 섞여 들어오면 그 문장은 읽지 않는다.
SUPPLY_RE = re.compile(r"수급|순매수|순매도|외국인|기관|개인|돈이\s?(?:머물|빠진|빠져|몰|들어)|돈은\s?(?:어디|어느)")
# 대본은 화면을 말로 설명하지 않는다(JJ 2026-09-16) — 따옴표 안 기사 제목은 쓰지 않으므로 그대로 잡는다.
SCREEN_RE = re.compile(r"막대|카드|도장|왼쪽|오른쪽|보세요|화면|그래프|(?<![가-힣])칸(?![가-힣])")
...
LINK_WORDS = re.compile(r"배경|짚었|꼽았|겹친|겹쳤|함께 다뤘|함께 짚|연결")


def _clean_ko(s: str) -> str:
    """기사 문장에서 수급 낱말로 오해되는 표현만 푼다('개인 AI 에이전트' → 'AI 에이전트')."""
    return re.sub(r"개인\s?(?=AI|비서|에이전트|용)", "", s or "")
...
def norm_news(news, w: dict) -> dict:
    ...
    ws = iso(w.get("week_start")) or ""
    events, releases = [], []
    for e in ev_raw + rel_raw:
        if not isinstance(e, dict):
            continue
        if e.get("verified") is False or e.get("use_in_script") is False:   # 검증 안 됐거나 대본에 쓰지 말라고 한 기사
            continue
        ...
        fact = _clean_ko(str(_first(e, ("spoken_ko", "fact_ko", "line_ko", "what", "summary_ko", "summary", "text")) or ""))
        ...
        events.append({"title": title, "fact": fact, "d": d0, "conf": conf, "prio": prio, "imp": float(e.get("importance") or 0),
                       ...
                       "politics": pol and not geo, "outlets": outs, "link": link, "blob": blob,
                       "key": f"{d0}|{title}", "kind": kl.strip(), "session": sess, "short": str(e.get("short_ko") or ""),
                       "what": _clean_ko(str(e.get("what") or fact)),
                       "bg": title.lstrip().startswith("[배경]") or bool(ws and sess and sess < ws),
                       # 두 곳 이상이 시장 움직임과 함께 짚었나 — 그래도 말은 '겹쳤습니다/같은 날'까지만 한다
                       "linked": len(outs) >= 2 and bool(LINK_WORDS.search(link)) and "확인되지 않" not in link})
    return {"events": events, "releases": releases}


REL_NAME = [(r"PPI|생산자물가", "생산자물가"), (r"CPI|소비자물가", "소비자물가"), (r"PCE|개인소비지출", "소비지출 물가"), (r"소매판매", "소매판매"),
            (r"GDP", "GDP"), (r"고용보고서|비농업|\d월 고용", "고용보고서"), (r"실업수당", "실업수당 청구"), (r"소비자심리", "소비자심리")]
...
# ── 사건(기사) 다루기: 이번 주 것 고르기 · 짧은 이름 · 같은 날 시장 반응 ──
KIND_RANK = {"data": 0, "geopolitics": 1, "oil": 1, "policy": 2, "rates": 3, "trade": 4, "politics": 4, "company": 5, "earnings": 5, "fx": 6}
_NUM_TOK = re.compile(r"(?<![\d.])[+\-−]?\d[\d,.]*(?![\d,.])(?!\s?(?:월|일|년|분기|시|개월|주년|차|거래일))"
                      r"\s?(?:%p|%포인트|%|bp|[만억조]\s?(?:달러|원|명|배럴|대)|[만억조]|달러|원|엔|명|건|척|배럴|배|포인트)?(?:으로|로)?(?=[\s·,…→/]|$)")
_BAD_END = re.compile(r"(?:지만|는데|하고|해서|하며|으며|고|며|서|→|~)$")


class Used:
    """장면끼리 같은 기사를 두 번 읽지 않게 — 훅·요일·자산 연결에서 쓴 기사, 업종·대형주 몫으로 남겨 둔 기사."""
    def __init__(self):
        self.keys: set[str] = set()
        self.reserved: set[str] = set()
        self.hook: dict | None = None
        self.hook_day = ""
        self.th = ""                 # 훅 주제 꾸밈말('기름값이 뛴')
        self.react = ""              # 훅에서 말한 자산
        self.rel_named = ""          # uw1 질문이 이름을 부른 지표 — uw2는 이름 없이 답한다
        self.odds: dict | None = None

    def add(self, e: dict | None) -> None:
        if e:
            self.keys.add(e["key"])

    def has(self, e: dict) -> bool:
        return e["key"] in self.keys


def ev_day(e: dict) -> str:
    return e.get("session") or e.get("d") or ""


def week_events(C: dict, N: dict, U: Used | None = None, allow_bg: bool = False, skip_reserved: bool = False) -> list[dict]:
    """이번 주(정규장 기준) high/medium 기사. 시황 요약(kind=market)은 사건이 아니라 빼고, 수급 낱말이 든 기사도 뺀다."""
    out = []
    for e in N["events"]:
        d0 = ev_day(e)
        if e["conf"] not in ("high", "medium") or not d0 or e.get("kind") == "market":
            continue
        if not allow_bg and (e.get("bg") or not (C["ws"] <= d0 <= C["we"])):
            continue
        if U and (U.has(e) or (skip_reserved and e["key"] in U.reserved)):
            continue
        out.append(e)
    return out


def rel_label(e: dict) -> str:
    """지표 기사면 '8월 소비자물가' 같은 이름."""
    t = e.get("title") or ""
    if e.get("kind") != "data" and not DATA_RE.search(t):
        return ""
    base = next((nm for rx, nm in REL_NAME if re.search(rx, t)), "")
    m = re.search(r"(\d{1,2})월", t)
    return f"{m.group(1)}월 {base}" if (base and m) else base


def ev_label(e: dict, C: dict | None = None) -> str:
    """사건의 짧은 이름(명사구, 26자 이하). short_ko → 지표 이름 → 금리 기사는 그날 10년물 → 제목 앞부분(숫자·인용 발언 뺌)."""
    if e.get("short"):
        return _clean_ko(e["short"])
    rl = rel_label(e)
    if rl:
        return rl
    if e.get("kind") == "rates" and C and C["S"].get("US10Y"):
        x = next((r for r in C["S"]["US10Y"]["days"] if r["d"] == ev_day(e)), None)
        if x and x.get("bp") is not None:
            return "국채 금리 " + ("급등" if x["bp"] >= 8 else "급락" if x["bp"] <= -8 else "상승" if x["bp"] > 0 else "하락")
    t = re.sub(r"^\s*\[[^\]]*\]\s*", "", e.get("title") or "")
    t = t.replace("美", "미국 ").replace("日", "일본 ").replace("中", "중국 ")
    for seg in re.split(r"…|\.{3}", t):
        s = re.sub(r"\([^)]*\)", "", seg)
        s = re.sub(r"\"[^\"]*\"|“[^”]*”", "", s)                 # 인용 발언은 이름이 못 된다
        s = re.sub(r"['‘’]", "", s)
        s = _NUM_TOK.sub("", s)
        s = re.sub(r"\s*[·,/→~]\s*(?=[·,/→~]|$)", "", s)
        s = re.sub(r"^\s*[·,/→~]\s*", "", s)
        s = re.sub(r"^([가-힣A-Za-z]{1,8}),\s*", r"\1의 ", s)
        s = re.sub(r"(?<=\s)(?:일부|소폭|인근)\s", "", " ".join(s.split()))
        s = " ".join(s.split()).strip(" ,·-")
        if len(re.findall("[가-힣]", s)) >= 6 and len(s) <= 26 and not _BAD_END.search(s) and not SUPPLY_RE.search(s) \
                and not forbidden.find(s):
            return _clean_ko(s)
    return ""


def ev_x(e: dict, C: dict | None = None) -> str:
    """말할 때의 이름 — 지표는 이름 그대로('8월 소비자물가가 나왔고'), 나머지는 '… 소식'."""
    lb = ev_label(e, C)
    if not lb:
        return ""
    return lb if rel_label(e) else f"{lb} 소식"


def ev_spoken(e: dict, max_len: int = 58) -> str | None:
    sp = spoken_fact(e["fact"], max_len=max_len)
    if sp and (SUPPLY_RE.search(sp) or SCREEN_RE.search(sp)):
        return None
    return sp


UP_W = re.compile(r"올랐|상승|급등|뛰었|뛴|오른|올라")
DN_W = re.compile(r"내렸|하락|급락|밀렸|내린|떨어|빠졌|내려")


def ev_move(e: dict, name: str) -> float | None:
    """기사 제목·사실 문장에 '이름 N%'가 있으면 그 기사가 말한 하루 등락(부호는 ± 또는 뒤따르는 동사)."""
    txt = f"{e.get('title', '')} {e.get('what', '')}"
    for m in re.finditer(re.escape(name) + r"(?:은|는|이|가|도|의)?\s?([+\-−])?(\d+(?:\.\d+)?)%(?!p|포인트)", txt):
        v = float(m.group(2))
        if m.group(1):
            return -v if m.group(1) in "-−" else v
        rest = re.split(r"\.(?!\d)", txt[m.end():m.end() + 60])[0]
        u, d_ = UP_W.search(rest), DN_W.search(rest)
        if u or d_:
            return v if (u and (not d_ or u.start() < d_.start())) else -v
    return None


def ev_title_movers(e: dict) -> list[tuple[str, float]]:
    """제목의 '암젠 -10%, 노바티스 -14%' → [(암젠, -10), (노바티스, -14)] (부호가 붙은 것만)."""
    out = []
    for m in re.finditer(r"([가-힣A-Za-z][가-힣A-Za-z0-9]*(?:\s?ADR)?)\s?([+\-−]\d+(?:\.\d+)?)%", e.get("title") or ""):
        nm = m.group(1)
        if re.search(r"지수$|정규장|시간외|장중|전년비|전월비|근원", nm):
            continue
        out.append((nm, float(m.group(2).replace("−", "-"))))
    return out


DAY_SCALE = {"WTI": 3.0, "US10Y": 8.0, "US2Y": 8.0, "SOX": 2.0, "SPX": 1.0, "IXIC": 1.2, "DJI": 1.0, "VIX": 12.0,
             "USDJPY": 0.8, "DXY": 0.5, "GOLD": 1.5, "USDKRW": 0.8}
KIND_ASSETS = {"oil": ["WTI"], "geopolitics": ["WTI"], "rates": ["US10Y"], "data": ["US10Y", "US2Y", "SPX"], "policy": ["US2Y", "SPX"],
               "fx": ["USDJPY", "DXY"], "company": ["SOX", "IXIC"], "earnings": ["IXIC"], "trade": ["SPX"], "politics": ["SPX"]}
BP_KEYS = ("US10Y", "US2Y")


def day_row(S: dict, k: str, d0: str) -> dict | None:
    s = S.get(k)
    return next((r for r in s["days"] if r["d"] == d0), None) if s else None


def day_move(S: dict, k: str, d0: str) -> float | None:
    r = day_row(S, k, d0)
    if not r:
        return None
    return r["bp"] if k in BP_KEYS else r["pct"]


def day_cross(S: dict, k: str, d0: str) -> float | None:
    """그날 넘은 둥근 선(WTI 10달러, 10년물 0.5%, 금 500달러)."""
    step = {"WTI": 10, "US10Y": 0.5, "GOLD": 500}.get(k)
    s = S.get(k)
    if not step or not s:
        return None
    i = next((j for j, r in enumerate(s["days"]) if r["d"] == d0), None)
    if i is None:
        return None
    prev = s["days"][i - 1]["close"] if i > 0 else s["prev"]
    return cross(prev, s["days"][i]["close"], step)


def react_of(C: dict, e: dict) -> dict | None:
    """기사 날 가장 크게 움직인 자산(기사가 꼽은 자산 + 종류별 기본 자산)."""
    S, d0, best = C["S"], ev_day(e), None
    for k in dict.fromkeys([a for a in e.get("assets") or [] if a in S] + KIND_ASSETS.get(e.get("kind", ""), [])):
        v = day_move(S, k, d0)
        if v is None or v == 0:
            continue
        L = day_cross(S, k, d0)
        sc = abs(v) / DAY_SCALE.get(k, 1.5) + (0.5 if L else 0)
        if not best or sc > best["sc"]:
            best = {"k": k, "d": d0, "v": v, "sc": sc, "L": L, "close": day_row(S, k, d0)["close"]}
    return best
...
SCENE_CAP = {"uw0": 160, "uw1": 270, "uw2": 260, "uw3": 230, "uw4": 240, "uw5": 230, "uw6": 360}
# 2026-09-17: 월요일 국장 수급을 보라는 말을 뺐다(JJ). uw6 은 '어떤 주였나' 한 줄 + 다음 주 일정 + 끝 멘트다.
...
# ── uw0 사건 훅(기사가 있는 주): 가장 큰 사건 + 같은 날 반응 + 한 주 지수와의 부딪힘 → 질문 ──
SAY = {"WTI": "WTI", "US10Y": "10년물 국채 금리", "US2Y": "2년물 국채 금리", "SOX": "반도체지수", "SPX": "S&P500", "IXIC": "나스닥",
       "DJI": "다우", "VIX": "공포지수 VIX", "USDJPY": "엔·달러 환율", "DXY": "달러 인덱스", "GOLD": "금값", "USDKRW": "원·달러 환율"}
# 주식에 부담으로 읽히는 방향(1 = 오르면 부담) — 부딪힘을 고를 때만 쓴다. 대본은 방향만 나란히 놓는다.
SHOCK_UP = {"WTI": 1, "US10Y": 1, "US2Y": 1, "VIX": 1, "DXY": 1, "GOLD": 1, "USDKRW": 1, "USDJPY": 0, "SOX": 0, "SPX": 0, "IXIC": 0, "DJI": 0}
THEME = {("WTI", 1): "기름값이 뛴", ("WTI", 0): "기름값이 내린", ("US10Y", 1): "금리가 뛴", ("US10Y", 0): "금리가 내린",
         ("US2Y", 1): "금리가 뛴", ("US2Y", 0): "금리가 내린", ("SOX", 0): "반도체가 흔들린", ("SOX", 1): "반도체가 오른",
         ("VIX", 1): "불안이 커진", ("VIX", 0): "불안이 가라앉은", ("SPX", 0): "주식이 밀린", ("SPX", 1): "주식이 오른",
         ("IXIC", 0): "기술주가 밀린", ("IXIC", 1): "기술주가 오른", ("DJI", 0): "주식이 밀린", ("DJI", 1): "주식이 오른",
         ("GOLD", 1): "금값이 뛴", ("GOLD", 0): "금값이 내린", ("USDJPY", 0): "엔화가 강해진", ("USDJPY", 1): "엔화가 약해진",
         ("DXY", 1): "달러가 강해진", ("DXY", 0): "달러가 약해진", ("USDKRW", 1): "원화가 약해진", ("USDKRW", 0): "원화가 강해진"}
H_EV = ["{wd}, {sp}", "{wd}엔 {sp}", "현지시간 {dd} {wd}, {sp}"]
H_EV_X = ["{wd}엔 {x_i_ga} 나왔습니다.", "{wd}, {x_i_ga} 전해졌습니다.", "현지시간 {dd} {wd}엔 {x_i_ga} 나왔습니다."]
H_RE_X = {"WTI": {1: ["같은 날 WTI는 하루 {amt} 뛰어 배럴당 {L}달러를 넘었습니다.", "그날 WTI가 {amt} 오르며 배럴당 {L}달러 선을 넘었습니다.",
                      "이날 기름값은 {amt} 뛰어, WTI 기준 {L}달러 위로 올라섰습니다."],
                  0: ["같은 날 WTI는 하루 {amt} 내려 배럴당 {L}달러 아래로 내려왔습니다.", "그날 WTI가 {amt} 내리며 배럴당 {L}달러 선 밑으로 갔습니다.",
                      "이날 기름값은 {amt} 내려, WTI 기준 {L}달러 아래로 내려섰습니다."]},
          "US10Y": {1: ["같은 날 10년물 국채 금리는 {amt} 올라 {L}%를 넘었습니다.", "그날 10년물 금리가 {amt} 뛰며 {L}% 선을 넘었습니다.",
                        "이날 10년물 금리는 {amt} 올라 {L}% 위로 올라섰습니다."],
                    0: ["같은 날 10년물 국채 금리는 {amt} 내려 {L}% 아래로 내려왔습니다.", "그날 10년물 금리가 {amt} 내리며 {L}% 선 밑으로 갔습니다.",
                        "이날 10년물 금리는 {amt} 내려 {L}% 아래로 내려섰습니다."]},
          "GOLD": {1: ["같은 날 금값은 {amt} 올라 온스당 {L}달러를 넘었습니다.", "그날 금값이 {amt} 뛰며 {L}달러 선을 넘었습니다.",
                       "이날 금값은 {amt} 올라 온스당 {L}달러 위로 올라섰습니다."],
                   0: ["같은 날 금값은 {amt} 내려 온스당 {L}달러 아래로 내려왔습니다.", "그날 금값이 {amt} 내리며 {L}달러 선 밑으로 갔습니다.",
                       "이날 금값은 {amt} 내려 온스당 {L}달러 아래로 내려섰습니다."]}}
H_RE = ["같은 날 {nm_eun} 하루 {amt} {va}습니다.", "그날 {nm_i_ga} 하루 만에 {amt} {vb}습니다.", "이날 {nm_eun} {amt} {vc}습니다."]
C_SAME_UP = ["그런데 {ix_eun} 한 주 오히려 {p} 올랐습니다.", "그런데도 한 주를 마친 {ix_eun} {p} 올라 있었습니다.", "그런데 {ix_eun} 한 주로 보면 {p} 올랐습니다."]
C_SMALL = ["그런데 한 주를 마친 {ix_eun} {p}만 내렸습니다.", "그런데 {ix}의 한 주 하락은 {p}에 그쳤습니다.", "그래도 {ix_eun} 한 주 {p} 내리는 데 그쳤습니다."]
C_BIG = ["{ix}도 한 주 {p} 밀렸습니다.", "한 주로는 {ix}도 {p} 내렸습니다.", "{ix} 역시 한 주 {p} 빠졌습니다."]
C_REL_DOWN = ["그런데 {ix_eun} 한 주로 보면 {p} 내렸습니다.", "그런데도 한 주를 마친 {ix_eun} {p} 내려 있었습니다.", "그런데 {ix}의 한 주는 {p} 하락이었습니다."]
C_REL_UP = ["{ix}도 한 주 {p} 올랐습니다.", "한 주로는 {ix}도 {p} 올랐습니다.", "{ix} 역시 한 주 {p} 올랐습니다."]
C_SELF = {1: ["그런데 한 주로는 오히려 {p} 올랐습니다.", "그래도 한 주를 합치면 {p} 올라 있었습니다.", "그런데 한 주 성적은 {p} 상승이었습니다."],
          0: ["그런데 한 주로는 오히려 {p} 내렸습니다.", "그래도 한 주를 합치면 {p} 내려 있었습니다.", "그런데 한 주 성적은 {p} 하락이었습니다."]}
Q_HOOK = [("{th} 한 주, 미국 주식은 어떻게 흘러갔을까요?", "{th} 한 주, 미국 주식은?"), ("그 한 주는 요일마다 어떻게 흘러갔을까요?", "그 한 주는 요일마다 어떻게?"),
          ("{th} 한 주엔 또 무슨 일이 있었을까요?", "{th} 한 주엔 무슨 일이?")]


def pick_hook_event(C: dict, N: dict) -> tuple[dict, dict] | tuple[None, None]:
    """중요도 + 그날 반응 크기(+ 둥근 선 돌파) + 신뢰도로 가장 큰 사건 하나."""
    best, br, bs = None, None, -1.0
    for e in week_events(C, N):
        if e["imp"] and e["imp"] < 2:
            continue
        r = react_of(C, e)
        if not r:
            continue
        sc = (e["imp"] or 2) + r["sc"] + (0.5 if e["conf"] == "high" else 0) + (0.3 if len(e["outlets"]) >= 2 else 0)
        if sc > bs:
            best, br, bs = e, r, sc
    return best, br


def build_uw0_event(C: dict, P: Pick, e: dict, r: dict, U: Used) -> tuple[str, list[str], dict]:
    S = C["S"]
    k, d0, v = r["k"], r["d"], r["v"]
    up = _up(v)
    kw = {"wd": wd(d0), "dd": dd(d0)}
    sp = ev_spoken(e, max_len=48)
    if sp and len(sp) <= 60:
        s1 = P("uw0.ev", H_EV, sp=sp, **kw)
    else:
        x = ev_x(e, C) or "큰 소식"
        s1 = P("uw0.evx", H_EV_X, x_i_ga=i_ga(x), **kw)
    amt = pp(v) if k in BP_KEYS else pct(v)
    L = r.get("L")
    if L and k in H_RE_X and _up(v) == _up(r["close"] - L):
        s2 = P(f"uw0.rex.{k}.{up}", H_RE_X[k][up], amt=amt, L=f"{L:g}")
    else:
        nm = SAY.get(k, k)
        s2 = P("uw0.re", H_RE, nm_eun=eun(nm), nm_i_ga=i_ga(nm), amt=amt, va=V_A[up], vb=V_B[up], vc=V_C[up])
    # 부딪힘: 사건 날 반응 방향 ↔ 한 주 지수
    ik = "SPX" if S.get("SPX") else "IXIC"
    ix, s3 = S.get(ik), ""
    if k in ("SPX", "IXIC", "DJI", "SOX") and S.get(k) and _up(S[k]["wp"]) != up and abs(S[k]["wp"]) >= 0.2:
        s3 = P(f"uw0.self.{_up(S[k]['wp'])}", C_SELF[_up(S[k]["wp"])], p=pct(S[k]["wp"]))
    elif ix and k != ik:
        nm = IDX_NAME[ik]
        ikw = {"ix": nm, "ix_eun": eun(nm), "p": pct(ix["wp"])}
        shock = (up == SHOCK_UP.get(k, 1))
        if shock:
            s3 = P("uw0.c.up", C_SAME_UP, **ikw) if ix["wp"] > 0.2 else P("uw0.c.small", C_SMALL, **ikw) if ix["wp"] > -1.0 \
                else P("uw0.c.big", C_BIG, **ikw)
        else:
            s3 = P("uw0.c.rdown", C_REL_DOWN, **ikw) if ix["wp"] < -0.2 else P("uw0.c.rup", C_REL_UP, **ikw) if ix["wp"] > 0.2 else ""
    wk = S.get(k)
    th = THEME.get((k, _up(wk["wp"] if wk else v)), "큰 소식이 이어진")
    q_tts, q_screen = P("uw0.q.ev", Q_HOOK, th=th)
    U.add(e)
    U.hook, U.hook_day, U.th, U.react = e, d0, th, k
    screen = {"event": {"d": d0, "label": ev_label(e, C), "kind": e.get("kind") or e["cluster"], "src": (e["outlets"] or [""])[0]},
              "react": {"key": k, "name": SAY.get(k, k), "d": d0, "v": v, "unit": "bp" if k in BP_KEYS else "%", "close": r["close"], "L": L},
              "idx": {"key": ik, "name": IDX_NAME[ik], "week_pct": ix["wp"] if ix else None}}
    return " ".join(x for x in (s1, s2, s3, q_tts) if x), q_screen, screen
...
# 요일별 사건 + 지수(JJ 2026-09-17: 사건과 엮어서 어떤 주였는지). 같은 방향으로 이어진 날은 한 문장으로 묶는다.
D_EV2 = ["{w1}엔 {x1}, {w2}엔 {x2_i_ga} 이어졌습니다.", "{w1} {x1_i_ga} 나왔고, {w2}엔 {x2_i_ga} 뒤따랐습니다.",
         "{x1_i_ga} 나온 {w1}에 이어, {w2}엔 {x2_i_ga} 나왔습니다."]
D_EV1 = ["{w1}엔 {x1_i_ga} 나왔습니다.", "{w1}, {x1_i_ga} 전해졌습니다.", "그 사이 {w1}엔 {x1_i_ga} 있었습니다."]
D_RUN = {0: ["{ix_eun} {a}부터 {thb}{b}까지 {nd} 내리 내렸습니다.", "{a}부터 {thb}{b}까지, {ix_eun} {nd} 연속 내렸습니다.",
             "{ix_eun} {a}부터 {thb}{b}까지 하루도 오르지 못했습니다."],
         1: ["{ix_eun} {a}부터 {thb}{b}까지 {nd} 내리 올랐습니다.", "{a}부터 {thb}{b}까지, {ix_eun} {nd} 연속 올랐습니다.",
             "{ix_eun} {a}부터 {thb}{b}까지 하루도 내리지 않았습니다."]}
D_ONE_EV = ["{wd}엔 {x_i_ga} 나왔고, {ix_eun} {mv}.", "{x_i_ga} 나온 {wd}, {ix_eun} {mv}.", "{wd}엔 {x_i_ga} 있었고, 이날 {ix_eun} {mv}."]
D_ONE = ["{wd}엔 {ix_i_ga} {mv}.", "{wd} {ix_eun} {mv}.", "이어 {wd}엔 {ix_i_ga} {mv}."]
D_WEEK = ["한 주로는 {ix_i_ga} {p1}, 나스닥이 {p2} {va}습니다.", "한 주를 합치면 {ix_eun} {p1}, 나스닥은 {p2} {vb}습니다.",
          "그래서 한 주 성적은 {ix} {p1}, 나스닥 {p2} {vn}이었습니다."]
D_WEEK_DIFF = ["한 주로는 {ix_i_ga} {p1} {v1}고, 나스닥은 {p2} {v2}습니다.", "한 주를 합치면 {ix_eun} {p1} {v1}지만, 나스닥은 {p2} {v2}습니다.",
               "그래서 한 주 성적은 {ix} {p1} {n1}, 나스닥 {p2} {n2}이었습니다."]
D_WEEK1 = ["한 주로는 {ix_i_ga} {p1} {va}습니다.", "한 주를 합치면 {ix_eun} {p1} {vb}습니다.", "그래서 {ix}의 한 주 성적은 {p1} {vn}이었습니다."]
D_SOX = ["반도체지수만 한 주 {p} {va}습니다.", "그 사이 반도체지수는 거꾸로 {p} {va}습니다.", "다만 반도체지수는 한 주 {p} {va}습니다."]
...
Q12_REL = ["그 {rwd}의 {rel}, 숫자는 어땠을까요?", "{rwd}에 나온 {rel}, 숫자는 어떻게 나왔을까요?", "그럼 {rwd} 발표된 {rel_eun} 어땠을까요?"]
Q12 = ["그 사이 금리는 어디로 갔을까요?", "이 한 주, 채권 금리는 어떻게 움직였을까요?", "그럼 같은 기간 금리는 어땠을까요?"]
...
def day_event(C: dict, N: dict, U: Used, d0: str) -> dict | None:
    """그날의 대표 사건 — 중요도 → 종류(지표 > 전쟁·유가 > 정책 > 금리 > 정치·통상 > 기업) → 신뢰도. 훅에서 쓴 기사는 뺀다."""
    evs = [e for e in week_events(C, N, U) if ev_day(e) == d0 and (not e["imp"] or e["imp"] >= 2) and ev_x(e, C)]
    evs.sort(key=lambda e: (-(e["imp"] or 2), KIND_RANK.get(e.get("kind", ""), 7), e["conf"] != "high"))
    return evs[0] if evs else None


def build_uw1(C: dict, N: dict, P: Pick, U: Used) -> tuple[list, dict]:
    S, parts, screen = C["S"], [], {"days": {}}
    for d0, nm in sorted(C["hol"].items()):
        ...
        break
    ik = "SPX" if S.get("SPX") else "IXIC" if S.get("IXIC") else ""
    ix = S.get(ik)
    if U.hook:
        screen["days"][U.hook_day] = {"label": ev_label(U.hook, C), "kind": U.hook.get("kind") or U.hook["cluster"]}
    if ix:
        nm = IDX_NAME[ik]
        ikw = {"ix": nm, "ix_eun": eun(nm), "ix_i_ga": i_ga(nm)}
        days = ix["days"]
        big_i = max(range(len(days)), key=lambda i: abs(days[i]["pct"] or 0))
        rs = runs(ix["signs"])
        for ri, (sg, a, b) in enumerate(rs):
            ds = [days[i]["d"] for i in range(a, b + 1)]
            evs = []
            for d0 in ds:
                e = day_event(C, N, U, d0)
                if e:
                    evs.append((d0, e))
            core = 0 if (a <= big_i <= b or ri == len(rs) - 1 or U.hook_day in ds) else 1
            if b > a:
                if evs:
                    (d1, e1), rest = evs[0], evs[1:3]
                    x1 = ev_x(e1, C)
                    if rest:
                        d2, e2 = rest[0]
                        x2 = ev_x(e2, C)
                        parts.append((1, P("uw1.ev2", D_EV2, w1=wd(d1), w2=wd(d2), x1=x1, x1_i_ga=i_ga(x1), x2_i_ga=i_ga(x2))))
                        used = [(d1, e1), (d2, e2)]
                    else:
                        parts.append((1, P("uw1.ev1", D_EV1, w1=wd(d1), x1_i_ga=i_ga(x1))))
                        used = [(d1, e1)]
                    for d0, e in used:
                        U.add(e)
                        screen["days"][d0] = {"label": ev_label(e, C), "kind": e.get("kind") or e["cluster"]}
                thb = f"{U.th} " if (U.th and U.hook_day == ds[-1]) else ""
                if sg == 0:
                    parts.append((core, P("uw1.flat", D_ONE, wd=f"{wd(ds[0])}부터 {wd(ds[-1])}까지", mv="거의 제자리였습니다", **ikw)))
                else:
                    parts.append((core, P(f"uw1.run{_up(sg)}", D_RUN[_up(sg)], a=wd(ds[0]), b=wd(ds[-1]), thb=thb,
                                          nd=ND.get(len(ds), f"{len(ds)}일"), **ikw)))
            else:
                d0 = ds[0]
                v = days[a]["pct"] or 0
                prev_run = rs[ri - 1] if ri > 0 else None
                if prev_run and prev_run[0] != 0 and sg != 0 and prev_run[0] != sg and prev_run[2] - prev_run[1] >= 1:
                    pn = prev_run[2] - prev_run[1] + 1
                    mv = f"{ND.get(pn, f'{pn}일')} {('오름세', '내림세')[_up(prev_run[0]) == 0]}를 끊고 {pct(v)} {V_A[_up(v)]}습니다"
                else:
                    mv = f"{pct(v)} {V_A[_up(v)]}습니다" if abs(v) >= 0.05 else "거의 제자리였습니다"
                e = evs[0][1] if evs else None
                if e:
                    x = ev_x(e, C)
                    parts.append((core, P("uw1.one.ev", D_ONE_EV, wd=wd(d0), x_i_ga=i_ga(x), mv=mv, **ikw)))
                    U.add(e)
                    screen["days"][d0] = {"label": ev_label(e, C), "kind": e.get("kind") or e["cluster"]}
                else:
                    parts.append((core, P("uw1.one", D_ONE, wd=wd(d0), mv=mv, **ikw)))
        nq = S.get("IXIC") if ik == "SPX" else None
        u1 = _up(ix["wp"])
        if nq and _up(nq["wp"]) == u1:
            parts.append((0, P("uw1.week", D_WEEK, p1=pct(ix["wp"]), p2=pct(nq["wp"]), va=V_A[u1], vb=V_B[u1], vn=("하락", "상승")[u1], **ikw)))
        elif nq:
            u2 = _up(nq["wp"])
            parts.append((0, P("uw1.week.diff", D_WEEK_DIFF, p1=pct(ix["wp"]), p2=pct(nq["wp"]), v1=V_A[u1], v2=V_A[u2],
                               n1=("하락", "상승")[u1], n2=("하락", "상승")[u2], **ikw)))
        else:
            parts.append((0, P("uw1.week1", D_WEEK1, p1=pct(ix["wp"]), va=V_A[u1], vb=V_B[u1], vn=("하락", "상승")[u1], **ikw)))
        sx = S.get("SOX")
        if sx and _up(sx["wp"]) != u1 and abs(sx["wp"]) >= 0.3 and U.react != "SOX":
            parts.append((2, P("uw1.sox", D_SOX, p=pct(sx["wp"]), va=V_A[_up(sx["wp"])]), "sox"))
    ...
    if rels and rels[0].get("d"):
        rn = rels[0]["name"]
        parts.append((0, P("uw1.q.rel", Q12_REL, rwd=wd(rels[0]["d"]), rel=rn, rel_eun=eun(rn))))
        U.rel_named = rn
    else:
        parts.append((0, P("uw1.q", Q12)))
    return parts, screen
...
Q23 = ["그럼 {names_eun} 이 소식들 사이에서 어떻게 움직였을까요?", "같은 한 주, {names_eun} 어디로 갔을까요?", "금리가 {vr2} 사이, {names_eun} 어땠을까요?"]
# uw1 질문이 지표 이름을 이미 불렀으면 이름 없이 숫자부터 답한다
REL_ANS = ["{vp}, 추정치 {exp_eul} {cmp}습니다.", "숫자는 {vp}, 시장이 점친 {exp}보다 {cmp2}습니다.", "{vp}, 추정치 {exp}보다 {cmp2}습니다."]
REL_ANS_EQ = ["{vp}, 시장 추정치와 같았습니다.", "숫자는 {vp}, 추정치 그대로였습니다.", "{vp}, 시장이 점친 숫자와 같았습니다."]
REL_ANS_NOEXP = ["숫자는 {vpp}.", "발표된 숫자는 {vpp}.", "{vpp}."]
ODDS = ["CME 페드워치 기준 {mo}금리 {dir} 확률은 {b}%에서 {a}%로 {v}습니다.", "발표 뒤 {mo}금리 {dir} 확률은 {b}%에서 {a}%로 {v2}습니다.",
        "시장이 보는 {mo}금리 {dir} 확률도 {b}%에서 {a}%로 {v}습니다."]
CLASH_HAWK = ["{dir} 확률이 올라간 날, {ix_eun} 오히려 {p} 올랐습니다.", "그런데 그 발표 날 {ix_eun} {p} 올랐습니다.", "그 발표 날, {ix_eun} 거꾸로 {p} 올랐습니다."]
CLASH_DOVE = ["{dir} 확률이 올라간 날, {ix_eun} 오히려 {p} 내렸습니다.", "그런데 그 발표 날 {ix_eun} {p} 내렸습니다.", "그 발표 날, {ix_eun} 거꾸로 {p} 내렸습니다."]
SAME_DAY = ["발표 날 {ix_eun} {p} {va}습니다.", "그날 {ix_eun} {p} {vb}습니다.", "이날 {ix_eun} {p} {vc}습니다."]
LINK_REL_DAY = ["10년물 금리가 가장 크게 {vn} {bwd}엔 {rel_i_ga} 나왔습니다.", "10년물 금리가 한 주 중 가장 크게 {vn} 날은 {rel_i_ga} 나온 {bwd_ieot}습니다.",
                "{rel_i_ga} 나온 {bwd}, 10년물 금리는 한 주 중 가장 크게 {va}습니다."]
BLACKOUT = ["연준 인사들은 회의를 앞두고 공개 발언을 삼가는 기간이었습니다.", "연준은 다음 회의를 앞두고 말을 아끼는 기간이었습니다.",
            "이번 주 연준 인사들은 규정에 따라 정책 발언을 삼갔습니다."]
ODDS_RE = re.compile(r"(인상|인하|동결)\s?확률[^.%]{0,30}?(\d{1,3}(?:\.\d)?)%\s?(?:에서|→)[^%]{0,20}?(\d{1,3}(?:\.\d)?)%")
INFL = ("소비자물가", "생산자물가", "소비지출 물가")
FX_SAY = {"WTI": "기름값", "GOLD": "금값", "USDJPY": "엔화", "USDKRW": "원화", "DXY": "달러"}


def week_odds(C: dict, N: dict) -> dict | None:
    """기사에 적힌 '인상 확률 A%에서 B%로' — 정책 기사 → 늦은 날 → 매체 수 순서로 하나."""
    cands = []
    for e in week_events(C, N):
        for txt in (e["what"], e["fact"], e["link"]):
            m = ODDS_RE.search(txt or "")
            if m:
                mo = re.search(r"(\d{1,2})월\s?\d{1,2}~\d{1,2}일|(\d{1,2})월[^.]{0,8}(?:FOMC|회의)", f"{e['what']} {e['link']}")
                cands.append({"dir": m.group(1), "before": float(m.group(2)), "after": float(m.group(3)), "d": ev_day(e),
                              "mo": f"{mo.group(1) or mo.group(2)}월 " if mo else "", "e": e, "src": (e["outlets"] or [""])[0],
                              "rank": (e.get("kind") != "policy", ev_day(e) and -int(ev_day(e).replace("-", "")), -len(e["outlets"]))})
                break
    return sorted(cands, key=lambda c: c["rank"])[0] if cands else None


def fx_moving(C: dict) -> list[str]:
    S = C["S"]
    items = {k: S[k] for k in FX_ORDER if S.get(k)}
    return sorted([k for k in items if abs(items[k]["wp"]) / FX_SCALE[k] >= 0.35], key=lambda k: -abs(items[k]["wp"]) / FX_SCALE[k])


def _rel_screen(r: dict) -> dict:
    basis = {"전년 대비": "1년 전 대비", "전월 대비": "한 달 전 대비"}.get(r.get("basis") or "", r.get("basis") or "")
    return {"key": "CORE" if r.get("core") else (r.get("base") or r["name"]), "name_ko": ("근원 물가" if r.get("core") else r["name"]),
            "period": basis + (" · 식품·에너지 제외" if r.get("core") else ""), "actual": r.get("actual"), "forecast": r.get("expected"),
            "unit": r.get("unit") or "%", "d": r.get("d"), "src": (r.get("outlets") or [""])[0]}


def build_uw2(C: dict, N: dict, P: Pick, hook: dict, U: Used) -> tuple[list, list[str], dict]:
    S, parts, screen = C["S"], [], {}
    r, r2 = S.get("US10Y"), S.get("US2Y")
    ik = "SPX" if S.get("SPX") else "IXIC"
    ix = S.get(ik)
    ikw = {"ix": IDX_NAME.get(ik, ""), "ix_eun": eun(IDX_NAME.get(ik, ""))}
    rels = week_releases(C, N)
    used_rel, hawk, dove = [], False, False
    if rels:
        kw = _rel_parts(rels[0])
        named = U.rel_named == rels[0]["name"]
        if kw.get("exp") is None:
            parts.append((0, P("uw2.rel1.noexp" + (".ans" if named else ""), REL_ANS_NOEXP if named else REL1_NOEXP, **kw)))
        elif named:
            parts.append((0, P("uw2.ans.eq" if kw["eq"] else "uw2.ans", REL_ANS_EQ if kw["eq"] else REL_ANS, **kw)))
        else:
            parts.append((0, P("uw2.rel1.eq" if kw["eq"] else "uw2.rel1", REL1_EQ if kw["eq"] else REL1, **kw)))
        used_rel.append(rels[0]["name"])
        if rels[0].get("base") in INFL and _surprise(rels[0]):
            hawk, dove = float(rels[0]["actual"]) > float(rels[0]["expected"]), float(rels[0]["actual"]) < float(rels[0]["expected"])
        for r_ in rels[1:2]:
            k2 = _rel_parts(r_)
            ...
            if r_.get("core") and r_.get("base") == rels[0].get("base"):
                ...
                parts.append((1, P("uw2.core.same" if same else "uw2.core", REL2_CORE_SAME if same else REL2_CORE, **k2)))
                if r_.get("base") in INFL and _surprise(r_) and not (hawk or dove):
                    hawk, dove = float(r_["actual"]) > float(r_["expected"]), float(r_["actual"]) < float(r_["expected"])
            else:
                parts.append((3, P("uw2.rel2.eq" if k2["eq"] else "uw2.rel2", REL2_EQ if k2["eq"] else REL2, **k2)))
            used_rel.append(r_["name"])
        screen["releases"] = [_rel_screen(x) for x in rels[:2]]
    od = week_odds(C, N)
    if od and abs(od["after"] - od["before"]) >= 3:
        rise = od["after"] > od["before"]
        same_day = bool(rels and rels[0].get("d") == od["d"])
        parts.append((1, P("uw2.odds", ODDS, ok=lambda v: same_day or "발표 뒤" not in v, mo=od["mo"], dir=od["dir"],
                           b=f"{od['before']:.0f}", a=f"{od['after']:.0f}", v=("내렸", "올랐")[rise], v2=("낮아졌", "높아졌")[rise])))
        U.add(od["e"])
        U.odds = od
        if rise and od["dir"] == "인상":
            hawk, dove = True, False
        elif rise and od["dir"] == "인하":
            hawk, dove = False, True
        screen["odds"] = {"before": od["before"], "after": od["after"], "label": f"{od['mo']}금리 {od['dir']} 확률", "src": "CME 페드워치"}
    if rels and ix and rels[0].get("d"):
        dv = day_move(S, ik, rels[0]["d"])
        if dv is not None:
            dkw = {**ikw, "p": pct(dv), "dir": od["dir"] if od else "인상"}
            if hawk and dv >= 0.3:
                parts.append((1, P("uw2.clash.hawk", CLASH_HAWK, ok=lambda v: bool(od) or "확률" not in v, **dkw)))
            elif dove and dv <= -0.3:
                parts.append((1, P("uw2.clash.dove", CLASH_DOVE, ok=lambda v: bool(od) or "확률" not in v, **dkw)))
            elif abs(dv) >= 0.3:
                u = _up(dv)
                parts.append((3, P("uw2.sameday", SAME_DAY, va=V_A[u], vb=V_B[u], vc=V_C[u], **dkw)))
    big = None
    if r:
        up = _up(r["wbp"])
        big = r["bigbp"]
        ...
        rp = 1 if rels else 0
        if hook["kind"] == "rate" and U.react not in BP_KEYS:
            parts.append((rp, P("uw2.r10.hook", R10_HOOK, **bkw)))
        elif abs(r["wbp"]) < 3:
            parts.append((rp, P("uw2.r10.flat", R10_FLAT, lvl=ylv(r["last"]))))
        else:
            parts.append((rp, P(f"uw2.r10.{up}", R10[up], pp=pp(r["wbp"]), lvl=ylv(r["last"]), lvl_ga=i_ga(ylv(r["last"])))))
        if r2 and abs(r2["wbp"]) >= 5:
            u2 = _up(r2["wbp"])
            parts.append((2, P("uw2.r2y", R2Y, pp=pp(r2["wbp"]), va=V_A[u2])))
        brel = next((x for x in N["releases"] if x.get("d") == big["d"] and not x.get("core") and x["name"] not in used_rel
                     and x["conf"] in ("high", "medium", "")), None)
        if brel and abs(big["bp"] or 0) >= 5:
            parts.append((2, P("uw2.link.relday", LINK_REL_DAY, rel_i_ga=i_ga(brel["name"]), **bkw)))
        elif rels and rels[0].get("d") == big["d"] and abs(big["bp"] or 0) >= 5:
            parts.append((2, P("uw2.link.rel", LINK_REL, **bkw)))
        parts.append((3, P("uw2.expl", EXPL10)))
    if any("블랙아웃" in e["title"] for e in week_events(C, N, allow_bg=True)):
        parts.append((3, P("uw2.blackout", BLACKOUT)))
    names = [FX_SAY[k] for k in fx_moving(C) if k in FX_SAY][:2] or ["달러와 기름값"]
    vr2 = "머무는" if not r or abs(r["wbp"]) < 3 else ("내리는", "오르는")[_up(r["wbp"])]
    parts.append((0, P("uw2.q", Q23, names_eun=eun(_and(names)), vr2=vr2)))
    return parts, used_rel, screen


# ── uw3 달러·엔·원·유가·금 — 사건 날 어떻게 움직였나(가장 크게 움직인 날의 기사와 나란히) ──
...
# 2026-09-17 JJ: 미국 주간은 수급을 말하지 않는다 — '국장 외국인 수급' 풀이를 뺐다.
DX1 = {1: ["주요 통화에 견준 달러 값, 달러 인덱스는 {p} 올랐습니다.",
           "유로·엔 같은 주요 통화에 견준 달러 값, 달러 인덱스는 {p} 올랐습니다.",
           "달러의 힘을 재는 달러 인덱스는 한 주 {p} 올랐습니다."],
       0: ["주요 통화에 견준 달러 값, 달러 인덱스는 {p} 내렸습니다.",
           "유로·엔 같은 주요 통화에 견준 달러 값, 달러 인덱스는 {p} 내렸습니다.",
           "달러의 힘을 재는 달러 인덱스는 한 주 {p} 내렸습니다."]}
...
GO_DOWN = ["금값은 {p} 내렸습니다. 불안할 때 찾는 금이 내렸다는 건 걱정이 줄었다는 뜻으로 읽히기도 합니다.",
           "금값은 {p} 내렸습니다. 금은 시장의 걱정을 비추는 숫자로 읽히기도 합니다.",
           "금은 {p} 밀렸습니다. 불안을 비추는 금값이 내려, 걱정이 누그러졌다는 풀이도 나옵니다."]
GO_UP = ["금값은 {p} 올랐습니다. 불안할 때 찾는 자산이라 걱정이 커졌다는 뜻으로 읽히기도 합니다.",
         "금값은 {p} 올랐는데, 금은 시장의 걱정을 비추는 숫자로 읽힙니다.", "금은 {p} 뛰었습니다. 불안할 때 찾는 자산이 올랐다는 뜻으로 읽히기도 합니다."]
...
Q34 = ["{th} 한 주, 시장 밖에선 또 어떤 일이 있었을까요?", "그 사이 정치와 기업 뉴스에선 어떤 일이 있었을까요?", "이 한 주, 다른 뉴스에선 어떤 일들이 있었을까요?"]
# 자산이 한 주 중 가장 크게 움직인 날 = 기사 날이면 나란히 놓는다(원인이라고 하지 않는다)
LINK_NAME = {"DXY": ("달러 인덱스", False), "USDKRW": ("원·달러 환율", False), "USDJPY": ("엔화 값", True), "GOLD": ("금값", False), "WTI": ("기름값", False)}
LINK_ASSET = ["{nm_i_ga} 가장 크게 {adn} {bwd}엔 {x_i_ga} 나왔습니다.", "{x_i_ga} 나온 {bwd}, {nm_eun} 한 주 중 가장 크게 {va}습니다.",
              "{nm_i_ga} 가장 크게 {adn} 날은 {x_i_ga} 나온 {bwd_ieot}습니다."]
PATH = ["{nm_eun} {a}까지 {va}다가 {b}엔 {p} {vb}습니다. 한 주로는 {wp} {vw}습니다.",
        "{nm_eun} {a}까지 {va}다가, {b} 하루 {p} {vb}습니다. 한 주 합계는 {wp} {vn}입니다.",
        "{b}엔 {nm_i_ga} {p} {vb}습니다. 그래도 한 주로는 {wp} {vw}습니다."]


def asset_event(C: dict, N: dict, U: Used, k: str) -> dict | None:
    """자산 k 가 한 주 중 가장 크게 움직인 날의 기사(그 자산을 꼽았거나 종류가 맞는 기사). 이미 읽은 기사는 뺀다."""
    s = C["S"].get(k)
    if not s or s["n"] < 2:
        return None
    bd = s["big"]["d"]
    evs = [e for e in week_events(C, N, U) if ev_day(e) == bd and ev_x(e, C)
           and (k in (e.get("assets") or []) or k in KIND_ASSETS.get(e.get("kind", ""), []))]
    evs.sort(key=lambda e: (k not in (e.get("assets") or []), -(e["imp"] or 2), e["conf"] != "high"))
    return evs[0] if evs else None


def build_uw3(C: dict, N: dict, P: Pick, hook: dict, U: Used) -> tuple[list, dict]:
    """달러·원·엔·금·유가 순서(마지막 유가가 다음 장면 뉴스로 이어진다). 움직인 항목은 숫자 하나 + 그 자산이 가장 크게 움직인 날의 기사,
    훅에서 말한 자산은 한 주 길(어디까지 오르다 언제 꺾였나). 한국 투자자 풀이 문장은 자리가 남을 때만(2~3순위)."""
    S = C["S"]
    items = {k: S[k] for k in FX_ORDER if S.get(k)}
    ...
    for k in FX_ORDER:
        ...
        if k in flats:
            if k == "DXY":
                parts.append((2, P("uw3.dxy.flat", DX_FLAT)))
            continue
        ...
        if U.react == k:
            rv = _rev(s)
            if rv and rv["last"]:
                bu = _up(rv["bday"]["pct"] or 0)
                parts.append((0, P(f"uw3.path.{up}", PATH, ok=lambda v, bu=bu: "그래도" not in v or bu != up,
                                   nm_eun=eun(nm), nm_i_ga=i_ga(nm), a=rv["a"], b=rv["b"], va=rv["va"], vb=V_A[bu], p=pct(rv["bday"]["pct"]),
                                   wp=pct(s["wp"]), vw=V_A[up], vn=("하락", "상승")[up])))
            if k == "WTI":
                parts.append((3, P(f"uw3.oil.hook.{up}", OI_HOOK[up], p=pct(s["wp"]))))
            continue
        if k == "USDKRW":
            chg = s["last"] - s["prev"]
            parts.append((pr + 1, P(f"uw3.krw1.{up}", K1[up], won=f"{abs(chg):.0f}원")))
        elif k == "USDJPY":
            parts.append((pr, P(f"uw3.jpy.{up}", J1[up], p=pct(s["wp"]))))
            parts.append((3, P(f"uw3.jpy.mean.{up}", J_MEAN[up])))
        elif k == "DXY":
            ...
            parts.append((pr, P(f"uw3.oil1.{up}", OI1[up], p=pct(s["wp"]))))
        e = asset_event(C, N, U, k)
        if e:
            bd = s["big"]["d"]
            bu = _up(s["big"]["pct"] or 0)
            bu = 1 - bu if inv else bu
            x = ev_x(e, C)
            parts.append((1, P("uw3.link", LINK_ASSET, nm_i_ga=i_ga(nm), nm_eun=eun(nm), adn=("내린", "오른")[bu], va=V_A[bu],
                               bwd=wd(bd), bwd_ieot=ieot(wd(bd)), x_i_ga=i_ga(x)), k))
            U.add(e)
            screen["links"][k] = {"d": bd, "label": ev_label(e, C), "kind": e.get("kind") or e["cluster"]}
...
# ── uw4 정치·지정학·기업: 기사로 확인된 것만(high/medium), 날짜·출처를 붙여 중립으로. 앞 장면에서 읽은 기사는 빼고 최대 셋 ──
W_FIRST = ["{dd} {wd}엔", "{wd}인 {dd}엔", "현지시간 {dd}엔"]
W_NEXT = ["{dd}엔", "이어 {dd}엔", "{wd}인 {dd}엔"]
W_BG = ["지난주 {wd}인 {dd}엔", "주가 시작되기 전인 {dd}엔", "지난주 {wd}엔"]
...
Q45 = ["그럼 이런 소식들 사이에서 업종과 대형주는 어떻게 움직였을까요?", "주식 안에선 어느 업종이 가장 크게 움직였을까요?",
       "그 한 주, 업종별로는 어디가 오르고 어디가 밀렸을까요?"]
CLUSTER_CAP = 2
UW4_KINDS = ("politics", "trade", "geopolitics", "company", "earnings")


def pick_events(C: dict, N: dict, U: Used, used_rel: list[str]) -> list[dict]:
    """정치·통상 하나, 전쟁·지정학 하나를 먼저 세우고 나머지는 중요도 순. 지표·금리·정책·환율 기사는 uw2·uw3 몫이라 뺀다.
    업종·대형주(uw5)가 쓰려고 남겨 둔 기사, 앞 장면에서 읽은 기사도 뺀다. 한 갈래는 최대 둘, 모두 셋까지."""
    lo, hi = _d(C["ws"]) - timedelta(days=3), _d(C["we"]) + timedelta(days=1)
    evs = []
    for e in week_events(C, N, U, allow_bg=True, skip_reserved=True):
        if not (lo <= _d(ev_day(e)) <= hi):
            continue
        k = e.get("kind", "")
        if k not in UW4_KINDS and not (k in ("", "oil") and e["cluster"] in ("geo", "pol", "co")):
            continue
        ...
        if sp:
            ...
            evs.append({**e, "spoken": sp, "outlets": outs})
    evs.sort(key=lambda e: (e["bg"], -(e["imp"] or 2), e["cluster"] not in ("geo", "pol"), e["conf"] != "high", e["d"]))
    seen, cnt, out = set(), {}, []

    def take(e):
        key = e["spoken"][:12]
        if key in seen or cnt.get(e["cluster"], 0) >= CLUSTER_CAP or len(out) >= 3 or e in out:
            return
        seen.add(key)
        cnt[e["cluster"]] = cnt.get(e["cluster"], 0) + 1
        out.append(e)
    for cl in ("pol", "geo"):
        first = next((e for e in evs if e["cluster"] == cl and not e["bg"]), None)
        if first:
            take(first)
    for e in evs:
        take(e)
    out.sort(key=lambda e: (e["bg"], -(e["imp"] or 2), e["cluster"] not in ("geo", "pol")))
    for i, e in enumerate(out):
        e["rank"] = i
    return sorted(out, key=lambda e: (ev_day(e), e["rank"]))


def build_uw4(C: dict, N: dict, P: Pick, U: Used, used_rel: list[str]) -> tuple[list, list[dict], dict]:
    evs = pick_events(C, N, U, used_rel)
    if not evs:
        return [], [], {}
    parts, n_dated, news = [], 0, []
    for i, e in enumerate(evs):
        d0 = e["d"] if e["bg"] else ev_day(e)
        d_, w_ = dd(d0), wd(d0)
        ...
        if own and outlet:
            ...
            if e["bg"]:
                when = P(f"uw4.whenbg{i}", W_BG, dd=d_, wd=w_)
            else:
                when = P("uw4.when0" if n_dated == 0 else f"uw4.when{i}", W_FIRST if n_dated == 0 else W_NEXT, dd=d_, wd=w_)
                n_dated += 1
            rep = reported(e["spoken"]) or ""
            avail = (lambda v, rep=rep, outlet=outlet, bg=e["bg"]: ("{rep}" not in v or bool(rep)) and ("{outlet}" not in v or bool(outlet))
                     and not (bg and "{dd} {fact}" in v))
        ...
        parts.append((min(e["rank"], 2), s, e["key"]))
        U.add(e)
        news.append({"d": d0, "title": ev_label(e, C) or e["title"], "line": "", "src": outlet, "tag": KIND_KO.get(e.get("kind", ""), ""),
                     "kind": e.get("kind") or e["cluster"], "key": e["key"]})
    oil = C["S"].get("WTI")
    if oil and abs(oil["wp"]) >= 3 and U.react != "WTI" and any(OIL_RE.search(e["blob"]) and not e["politics"] for e in evs):
        parts.append((1, P(f"uw4.link.oil{_up(oil['wp'])}", LINK_OIL[_up(oil["wp"])])))
    return parts, evs, {"news": news}


KIND_KO = {"geopolitics": "전쟁·분쟁", "politics": "정치", "trade": "관세·통상", "company": "기업", "earnings": "실적", "policy": "연준·정책",
           "rates": "금리", "data": "지표", "oil": "유가", "fx": "환율"}
...
TH_REPLY_CH = ["주말엔 미국 한 주, 평일엔 저녁 5시에 한국 시장 마감 이야기를 해.",
               "주말에는 미국 한 주, 평일에는 저녁 5시 한국 시장 마감 이야기야.",
               "평일 저녁 5시엔 한국 시장 마감, 주말엔 미국 한 주 이야기야."]
